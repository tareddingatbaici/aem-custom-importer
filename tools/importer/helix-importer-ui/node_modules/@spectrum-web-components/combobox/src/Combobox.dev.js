...
export class Combobox extends Textfield {
  ...
  scrollToActiveDescendant() {
    ...
    const activeEl = this.shadowRoot.querySelector(
      `#${(_a = this.activeDescendant) == null ? void 0 : _a.value}`
    );
    if (activeEl) {
  ...
  activateNextDescendant() {
    ...
    const nextActiveIndex = (this.availableOptions.length + activeIndex + 1) % this.availableOptions.length;
  ...
  activatePreviousDescendant() {
    ...
    const previousActiveIndex = (this.availableOptions.length + activeIndex - 1) % this.availableOptions.length;
